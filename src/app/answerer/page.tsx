...
export default function AnswererPage() {
  ...
  const getRandomTopic = () => {
    ...
    const currentParticipants = hints.map(h => h.player).filter((name, index, arr) => arr.indexOf(name) === index);
    const playerHints = currentParticipants.map((name, index) => ({
      id: index + 1,
      text: '',
      player: name,
      isDuplicate: false,
    ...
    setAnswerer(selectNextAnswerer());
    setShowResult(false);
    setIsCorrect(false);
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-400 via-purple-400 to-pink-400 p-3 sm:p-4">
      <div className="max-w-md mx-auto sm:max-w-4xl">
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl p-4 sm:p-8 mb-4 sm:mb-6">
          ...
          {hintsPublished && validHints.length > 0 && (
            <div className="mb-8">
              <div className="text-center mb-6">
                <h3 className="text-xl font-bold text-gray-800 mb-4">あなたの回答</h3>
                <input
                  type="text"
                  value={userAnswer}
                  onChange={(e) => setUserAnswer(e.target.value)}
                  placeholder="答えを入力してください..."
                  className="w-full max-w-md px-4 py-3 text-lg rounded-xl border-2 border-gray-300 focus:border-blue-400 focus:outline-none"
                  disabled={showResult}
                />
              </div>
              ...
              <div className="text-center">
                <button
                  onClick={() => {
                    setShowResult(true);
                    setIsCorrect(userAnswer === topic);
                    setGameEnded(true);
                  }}
                  disabled={!userAnswer.trim() || showResult}
                  className="bg-gradient-to-r from-green-400 to-blue-500 hover:from-green-500 hover:to-blue-600 disabled:from-gray-300 disabled:to-gray-400 text-white px-8 py-3 rounded-xl font-bold text-lg shadow-lg transition-all transform hover:scale-105 disabled:scale-100 disabled:cursor-not-allowed"
                >
                  回答する
                </button>
              </div>
          ...
          {showResult && (
            <div className="text-center">
              <div className={`inline-block px-8 py-4 rounded-xl font-bold text-xl mb-4 ${
                isCorrect 
